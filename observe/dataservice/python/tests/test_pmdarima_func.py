...
@pytest.fixture
def gateway() -> JavaGateway:
    return JavaGateway()


def test_predict_target_in_event(gateway: JavaGateway):
    func = PmdArimaFunction(gateway)
    res = func.compute(
    ...
    assert res.value == pytest.approx(135032.70443054268)
    assert res.lower == pytest.approx(112346.85779933422)
    assert res.upper == pytest.approx(157718.55106175115)


def test_predict_target_outside_event(gateway: JavaGateway):
    func = PmdArimaFunction(gateway)
    res = func.compute(
            values=[89119.0, 117282.0, 78701.0, 63456.0, 63731.0, 78161.0,
                    78581.0, 97605.0, 122480.0, 73446.0, 50475.0, 54593.0,
            ...
            timestamps=[1631836800000, 1631923200000, 1632009600000,
                        ...
                        1639094400000, 1639180800000, 1639267200000,
                        1639353600000, 1639440000000, 1639526400000, ],
            alpha=0.05, p=1, d=0, q=0, seasonal_P=2, seasonal_D=1, seasonal_Q=0, m=7,
            eventPeriods=[
    ...
    assert res.value == pytest.approx(135032.70443054268)
    assert res.lower == pytest.approx(115500.96977330235)
    assert res.upper == pytest.approx(154564.4390883048)


def test_predict_with_prior_events(gateway: JavaGateway):
    func = PmdArimaFunction(gateway)
    with open("baseline.txt", "rt") as f:
        baseline = json.loads(f.read())
    with open("timestamps.txt", "rt") as f:
        timestamps = json.loads(f.read())
    with open("priors.json", "rt") as f:
        raw_priors = json.loads(f.read())
        priors = []
        for p in raw_priors:
            timestamp = int(list(p.keys())[0])
            priors.append({timestamp: list(p.values())[0]})
    res = func.compute(
            values=baseline,
    ...
    assert res.value == pytest.approx(253631.406962)
