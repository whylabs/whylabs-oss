...
public class MonotonicCalculationDouble extends BaseCalculationV3<Double, FixedCalculationResult> {
  private MonotonicCalculationConfig config;

  public MonotonicCalculationDouble(
  ...
  public FixedCalculationResult calculate(
    ...
    TreeMap<Long, Double> baselineWithTarget = new TreeMap<>();
    for (val p : baseline) {
    ...
    if (config.getDirection().equals(MonotonicDirection.INCREASING)) {
      builder.thresholdType(ThresholdType.upper);
    } else {
      builder.thresholdType(ThresholdType.lower);
    }
