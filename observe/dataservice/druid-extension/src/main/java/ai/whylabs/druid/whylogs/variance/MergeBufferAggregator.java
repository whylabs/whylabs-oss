...
public class MergeBufferAggregator implements BufferAggregator {
   ...
   * looks up the object in this map. After updates, the map and buffer are both updated.
   *
   ...
   * buffers.
   */
  private final IdentityHashMap<ByteBuffer, Int2ObjectMap<VarianceTracker>> unions =
      new IdentityHashMap<>();

  private final ColumnValueSelector<VarianceTracker> selector;
  ...
  public synchronized void aggregate(final ByteBuffer buf, final int position) {
    VarianceTracker union = unions.get(buf).get(position);
    ...
    try {
      if (object instanceof VarianceTracker) {
        VarianceTracker col = (VarianceTracker) object;
        if (union == null) {
          union = col;
        } else {
      ...
      if (encoded.length > size) {
        // be extra paranoid to perhaps catch buffer overflows.
        log.error(
            "VarianceTracker exceeded maximum size {} > {}, {}",
            encoded.length,
            size,
            union.toString());
      } else {
  ...
  public synchronized void relocate(
    ...
    VarianceTracker union = unions.get(oldBuffer).get(oldPosition);
    putUnion(newBuffer, newPosition, union);
    final Int2ObjectMap<VarianceTracker> map = unions.get(oldBuffer);
  ...
  private void putUnion(final ByteBuffer buffer, final int position, final VarianceTracker union) {
    Int2ObjectMap<VarianceTracker> map =
