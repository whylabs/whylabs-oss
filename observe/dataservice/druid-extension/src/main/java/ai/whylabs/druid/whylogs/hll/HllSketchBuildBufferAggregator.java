...
/**
 * This aggregator builds sketches from raw data. The input column can contain identifiers of type
 * string, char[], byte[] or any numeric type.
 */
public class HllSketchBuildBufferAggregator implements BufferAggregator {
  ...
  private final IdentityHashMap<ByteBuffer, Int2ObjectMap<HllSketch>> sketchCache =
      new IdentityHashMap<>();
  private final Striped<ReadWriteLock> stripedLock = Striped.readWriteLock(NUM_STRIPES);

  /**
   * Used by {@link #init(ByteBuffer, int)}. We initialize by copying a prebuilt empty HllSketch
   * image. {@link HllSketchMergeBufferAggregator} does something similar, but different enough that
   * we don't share code. The "build" flavor uses {@link HllSketch} objects and the "merge" flavor
  ...
  public HllSketchBuildBufferAggregator(
    ...
    //noinspection ResultOfObjectAllocationIgnored (HllSketch writes to "emptySketch" as a side
  ...
  private WritableMemory getMemory(final ByteBuffer buf) {
    return memCache.computeIfAbsent(
        buf, b -> WritableMemory.writableWrap(b, ByteOrder.LITTLE_ENDIAN));
  }

  /**
   * In very rare cases sketches can exceed given memory, request on-heap memory and move there. We
   * need to identify such sketches and reuse the same objects as opposed to wrapping new memory
   * regions.
   */
  @Override
  ...
  private void putSketchIntoCache(
      final ByteBuffer buf, final int position, final HllSketch sketch) {
    final Int2ObjectMap<HllSketch> map =
        sketchCache.computeIfAbsent(buf, b -> new Int2ObjectOpenHashMap<>());
  ...
  public void inspectRuntimeShape(RuntimeShapeInspector inspector) {
    ...
    // lgK should be inspected because different execution paths exist in HllSketch.update() that is
    // called from
