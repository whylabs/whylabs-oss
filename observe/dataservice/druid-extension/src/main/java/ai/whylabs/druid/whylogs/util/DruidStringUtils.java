...
// Forked from druid core so we can drop the dependency
public class DruidStringUtils {
  ...
  public static String fromUtf8(final ByteBuffer buffer) {
    return DruidStringUtils.fromUtf8(buffer, buffer.remaining());
  }
  ...
  /**
   * Encodes "string" into the buffer "byteBuffer", using no more than the number of bytes remaining
   * in the buffer. Will only encode whole characters. The byteBuffer's position and limit may be
   * changed during operation, but will be reset before this method call ends.
   *
   * @return the number of bytes written, which may be shorter than the full encoded string length
  ...
  public static int toUtf8WithLimit(final String string, final ByteBuffer byteBuffer) {
    ...
    try {
      ...
      // No reason to look at the CoderResult from the "encode" call; we can tell the number of
  ...
  /**
   * Formats the string as {@link #format(String, Object...)}, but instead of failing on illegal
   * format, returns the concatenated format string and format arguments. Should be used for
   * unimportant formatting like logging, exception messages, typically not directly.
   */
  ...
  /**
   * Encodes a String in application/x-www-form-urlencoded format, with one exception: "+" in the
   * encoded form is replaced with "%20".
   *
   * <p>application/x-www-form-urlencoded encodes spaces as "+", but we use this to encode non-form
   * data as well.
   *
   * @param s String to be encoded
   * @return application/x-www-form-urlencoded format encoded String, but with "+" replaced with
   *     "%20".
   */
  @Nullable
  ...
  /**
   * Removes all occurrences of the given char from the given string. This method is an optimal
   * version of {@link String#replace(CharSequence, CharSequence) s.replace("c", "")}.
   */
  ...
  /**
   * Replaces all occurrences of the given char in the given string with the given replacement
   * string. This method is an optimal version of {@link String#replace(CharSequence, CharSequence)
  ...
  /**
   * Replaces all occurrences of the given target substring in the given string with the given
   * replacement string. This method is an optimal version of {@link String#replace(CharSequence,
   * CharSequence) s.replace(target, replacement)}.
   */
  public static String replace(String s, String target, String replacement) {
    // String.replace() is suboptimal in JDK8, but is fixed in JDK9+. When the minimal JDK version
    // supported by Druid is
    // JDK9+, the implementation of this method should be replaced with simple delegation to
    // String.replace(). However,
    // the method should still be prohibited to use in all other places except this method body,
    // because it's easy to
    // suboptimally call String.replace("a", "b"), String.replace("a", ""), String.replace("a",
    // "abc"), which have
  ...
  /**
   * Returns the given string if it is non-null; the empty string otherwise. This method should only
   * be used at places where null to empty conversion is irrelevant to null handling of the data.
   *
  ...
  /**
   * Returns the given string if it is nonempty; {@code null} otherwise. This method should only be
   * used at places where null to empty conversion is irrelevant to null handling of the data.
   *
   * @param string the string to test and possibly return
  ...
  /**
   * Convert an input byte array into a newly-allocated byte array using the {@link Base64} encoding
   * scheme
   *
  ...
  /**
   * Decode an input byte array using the {@link Base64} encoding scheme and return a
  ...
  /**
   * Decode an input string using the {@link Base64} encoding scheme and return a newly-allocated
   * byte array
   *
  ...
  /**
   * Returns a string whose value is the concatenation of the string {@code s} repeated {@code
   * count} times.
   *
   * <p>If count or length is zero then the empty string is returned.
   *
   * <p>This method may be used to create space padding for formatting text or zero padding for
   * formatting numbers.
   *
   * @param count number of times to repeat
   * @return A string composed of this string repeated {@code count} times or the empty string if
   *     count or length is zero.
  ...
  /**
   * Returns the string left-padded with the string pad to a length of len characters. If str is
   * longer than len, the return value is shortened to len characters. This function is migrated
   * from flink's scala function with minor refactor
   ...
   * - Modified to handle empty pad string. - Padding of negative length return an empty string.
   *
   * @param base The base string to be padded
   * @param len The length of padded string
   * @param pad The pad string
   * @return the string left-padded with pad to a length of len or null if the pad is empty or the
   *     len is less than 0.
   */
  @Nonnull
  ...
  /**
   * Returns the string right-padded with the string pad to a length of len characters. If str is
   * longer than len, the return value is shortened to len characters. This function is migrated
   * from flink's scala function with minor refactor
   * https://github.com/apache/flink/blob/master/flink-table/flink-table-planner/src/main/scala/org/apache/flink/table/runtime/functions/ScalarFunctions.scala
   * - Modified to handle empty pad string. - Modified to only copy the pad string if needed (this
   * implementation mimics lpad). - Padding of negative length return an empty string.
   *
   * @param base The base string to be padded
   * @param len The length of padded string
   * @param pad The pad string
   * @return the string right-padded with pad to a length of len or null if the pad is empty or the
   *     len is less than 0.
   */
  @Nonnull
  ...
  /**
   * Returns the string truncated to maxBytes. If given string input is shorter than maxBytes, then
   * it remains the same.
   *
  ...
  public static String chop(@Nullable final String s, final int maxBytes) {
    if (s == null) {
      ...
      final int len = DruidStringUtils.toUtf8WithLimit(s, ByteBuffer.wrap(bytes));
