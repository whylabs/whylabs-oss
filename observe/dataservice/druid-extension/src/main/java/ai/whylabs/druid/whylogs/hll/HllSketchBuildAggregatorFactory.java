...
/**
 * This aggregator factory is for building sketches from raw data. The input column can contain
 * identifiers of type string, char[], byte[] or any numeric type.
 */
public class HllSketchBuildAggregatorFactory extends HllSketchAggregatorFactory {
  ...
  public BufferAggregator factorizeBuffered(final ColumnSelectorFactory columnSelectorFactory) {
    ...
    return new HllSketchBuildBufferAggregator(
        selector, getLgK(), TgtHllType.valueOf(getTgtHllType()), getMaxIntermediateSize());
  }

  /**
   * For the HLL_4 sketch type, this value can be exceeded slightly in extremely rare cases. The
   * sketch will request on-heap memory and move there. It is handled in
   * HllSketchBuildBufferAggregator.
   */
  @Override
  public int getMaxIntermediateSize() {
    return HllSketch.getMaxUpdatableSerializationBytes(
        getLgK(), TgtHllType.valueOf(getTgtHllType()));
  }
}
