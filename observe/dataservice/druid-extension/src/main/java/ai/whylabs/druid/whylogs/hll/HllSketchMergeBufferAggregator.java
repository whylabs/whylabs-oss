...
/** This aggregator merges existing sketches. The input column must contain {@link HllSketch} */
public class HllSketchMergeBufferAggregator implements BufferAggregator {
   ...
   * {@link HllSketchBuildBufferAggregator} does something similar, but different enough that we
   * don't share code. The "build" flavor uses {@link HllSketch} objects and the "merge" flavor uses
   * {@link Union} objects.
   */
  ...
  public HllSketchMergeBufferAggregator(
    ...
    //noinspection ResultOfObjectAllocationIgnored (Union writes to "emptyUnion" as a side effect of
    // construction)
  ...
  public void init(final ByteBuffer buf, final int position) {
    ...
    //  - Unlike the HllSketch objects used by HllSketchBuildBufferAggregator, our Union objects
    // never exceed the
  ...
  public void aggregate(final ByteBuffer buf, final int position) {
    ...
    final WritableMemory mem =
        WritableMemory.writableWrap(buf, ByteOrder.LITTLE_ENDIAN).writableRegion(position, size);
    final Lock lock =
  ...
  public Object get(final ByteBuffer buf, final int position) {
    final WritableMemory mem =
        WritableMemory.writableWrap(buf, ByteOrder.LITTLE_ENDIAN).writableRegion(position, size);
    final Lock lock =
        stripedLock.getAt(HllSketchBuildBufferAggregator.lockIndex(position)).readLock();
    lock.lock();
    try {
      final Union union = Union.writableWrap(mem);
  ...
  public void inspectRuntimeShape(RuntimeShapeInspector inspector) {
    ...
    // lgK should be inspected because different execution paths exist in Union.update() that is
    // called from
