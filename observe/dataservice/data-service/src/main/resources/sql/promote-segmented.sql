...
with deleted as (delete from whylabs.profiles_segmented_staging where org_id = ? and dataset_id = ?  and dataset_timestamp >= ? and dataset_timestamp <= ? and segment_text = ? returning *),
...
select org_id, dataset_id, ARRAY_AGG(column_name) as column_names, dataset_timestamp, min(earliest_ingest_timestamp) as earliest_ingest_timestamp, now(), ? from columns group by org_id, dataset_id, dataset_timestamp
