...
ALTER TABLE whylabs.analysis_anomalies ADD COLUMN IF NOT EXISTS frequent_string_sample_count jsonb;
ALTER TABLE whylabs.analysis_non_anomalies ADD COLUMN IF NOT EXISTS frequent_string_sample_count jsonb;

ALTER TABLE whylabs.analysis_anomalies_pg ADD COLUMN IF NOT EXISTS frequent_string_sample_count jsonb;
ALTER TABLE whylabs.analysis_non_anomalies_pg ADD COLUMN IF NOT EXISTS frequent_string_sample_count jsonb;
ALTER TABLE whylabs.analysis_adhoc ADD COLUMN IF NOT EXISTS frequent_string_sample_count jsonb;
...
create or replace view whylabs.analysis_pg as select * from whylabs.analysis_anomalies_pg union all select * from whylabs.analysis_non_anomalies_pg;

-- Helper method to convert json arrays to int arrays
...
-- -- Same as what we had previously, You have to replace the whole function. The only thing that changed
-- -- is the where clause on the delete
...
BEGIN
    ...
    -- with the latest.
    delete from whylabs.analysis_anomalies where analysis_anomalies.analysis_id = (NEW.json_blob->>'analysis_id')::uuid  and analysis_anomalies.dataset_timestamp = to_timestamp((NEW.json_blob->>'dataset_timestamp')::numeric/ 1000);
    delete from whylabs.analysis_non_anomalies where analysis_non_anomalies.analysis_id = (NEW.json_blob->>'analysis_id')::uuid and analysis_non_anomalies.dataset_timestamp = to_timestamp((NEW.json_blob->>'dataset_timestamp')::numeric/ 1000);
    ...
    with r as (
        select
            ...
            (NEW.json_blob->'frequent_string_sample_count') as frequent_string_sample_count,
         ...
         step2 as (insert into whylabs.analysis_anomalies (org_id, dataset_id, column_name, id, run_id, analysis_id, dataset_timestamp, creation_timestamp, monitor_ids, seasonal_lambda_keep, seasonal_adjusted_prediction, seasonal_replacement, drift_metric_value, diff_metric_value, drift_threshold, diff_threshold, threshold_absolute_upper, threshold_absolute_lower,
                                                           ...
                                                           child_analyzer_ids, child_analysis_ids, parent, frequent_string_sample_count)
