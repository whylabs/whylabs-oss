alter table whylabs.analysis_adhoc add column if not exists child_analyzer_ids text[];
alter table whylabs.analysis_adhoc add column if not exists child_analysis_ids text[];
alter table whylabs.analysis_adhoc add column if not exists parent bool;

COMMENT ON COLUMN whylabs.analysis_adhoc.parent IS 'This is a composite analyzer';

-- Only change from 75-analyzer-result-bulk-fix.sql is adding child_analyzer_ids, child_analysis_ids, parent
