...
-- drop view first, or it will prevent dropping columns
drop view if exists whylabs.analysis;

alter table whylabs.analysis_non_anomalies drop column if exists diff_lower;
alter table whylabs.analysis_anomalies drop column if exists diff_lower;
alter table whylabs.analysis_adhoc drop column if exists diff_lower;
alter table whylabs.analysis_non_anomalies drop column if exists diff_upper;
alter table whylabs.analysis_anomalies drop column if exists diff_upper;
alter table whylabs.analysis_adhoc drop column if exists diff_upper;
...
-- Same as what we had previously, just removing diff_lower/diff_upper. You have to replace the whole function
    ...
    with r as (
        select
         ...
         step2 as (insert into whylabs.analysis_anomalies (org_id, dataset_id, column_name, id, run_id, analysis_id, dataset_timestamp, creation_timestamp, monitor_ids, seasonal_lambda_keep, seasonal_adjusted_prediction, seasonal_replacement, drift_metric_value, diff_metric_value, drift_threshold, diff_threshold, threshold_absolute_upper, threshold_absolute_lower,
                                                           ...
                                                           child_analyzer_ids, child_analysis_ids, parent)
