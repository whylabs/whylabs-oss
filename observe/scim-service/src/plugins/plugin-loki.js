...
scimgateway.authPassThroughAllowed = true; // true enables auth passThrough (no scimgateway authentication). scimgateway instead includes ctx (ctx.request.header) in plugin methods. Note, requires plugin-logic for handling/passing ctx.request.header.authorization to be used in endpoint communication
...
scimgateway.getUsers = async (baseEntity, getObj, attributes, ctx) => {
  ...
  if (getObj.operator) {
    // convert to plugin supported syntax
    switch (getObj.operator) {
      ...
      default:
        if (!validFilterOperators.includes(getObj.operator)) {
          const err = new Error(
            `${action} error: filter operator '${getObj.operator}' is not valid, valid operators for this endpoint are: ${validFilterOperators}` +
              ',co,ge,le,sw,ew',
          );
  ...
  }); // all attributes included - virtual attribute groups automatically handled by scimgateway
...
scimgateway.modifyUser = async (baseEntity, id, attrObj, ctx) => {
  ...
  if (res.length > 1)
    throw new Error(`${action} error: user id=${id} - user is not unique, more than one have been found`);
  const userObj = res[0];
  ...
  for (const key in attrObj) {
    if (Array.isArray(attrObj[key])) {
      ...
      addArr.forEach((el) => {
        if (Object.prototype.hasOwnProperty.call(el, 'primary')) {
          if (el.primary === true || (typeof el.primary === 'string' && el.primary.toLowerCase() === 'true')) {
            ...
            if (index >= 0) {
              if (key === 'roles') userObj[key].splice(index, 1);
      ...
      for (const el in attrObj[key]) {
        ...
        if (attrObj[key][el].operation && attrObj[key][el].operation === 'delete') {
          // delete multivalue
          let type = el;
      ...
      if (typeof attrObj[key] !== 'object' || attrObj[key] === null) {
        ...
        for (const sub in attrObj[key]) {
          ...
          if (sub === 'attributes' && Array.isArray(attrObj[key][sub])) {
            ...
            if (Object.prototype.hasOwnProperty.call(attrObj[key][sub], 'value') && attrObj[key][sub].value === '')
...
scimgateway.createGroup = async (baseEntity, groupObj, ctx) => {
  ...
  if (groupObj.externalId) groupObj.id = groupObj.externalId;
  // for loki-plugin (scim endpoint) id is mandatory and set to displayName
...
scimgateway.modifyGroup = async (baseEntity, id, attrObj, ctx) => {
  ...
  if (res.length > 1)
    throw new Error(`${action} error: group id=${id} - group is not unique, more than one have been found`);
  const groupObj = res[0];
  ...
  if (usersNotExist.length > 0)
    throw new Error(
      `${action} error: failed for id=${groupObj.id} - includes none existing members: ${usersNotExist.toString()}`,
    );
  return null;
};
