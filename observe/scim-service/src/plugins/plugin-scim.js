...
scimgateway.getUsers = async (baseEntity, getObj, attributes, ctx) => {
  ...
  if (getObj.operator) {
    if (getObj.operator === 'eq' && ['id', 'userName', 'externalId'].includes(getObj.attribute)) {
      ...
      if (getObj.attribute === 'id') path = `/Users/${getObj.value}?attributes=${attributes.join()}`;
      // GET /Users/bjensen?attributes=
      else
        path = `/Users?filter=${getObj.attribute} eq "${getObj.value}"${
          attributes.length > 0 ? '&attributes=' + attributes.join() : ''
        }`; // GET /Users?filter=userName eq "bjensen"&attributes=userName,active,name.givenName,name.familyName,name.formatted,title,emails,phoneNumbers,entitlements
      ...
      path = `/Users?filter=${getObj.attribute} ${getObj.operator} "${getObj.value}"${
        attributes.length > 0 ? '&attributes=' + attributes.join() : ''
      }`;
    }
...
scimgateway.getGroups = async (baseEntity, getObj, attributes, ctx) => {
  ...
  if (getObj.operator) {
    if (getObj.operator === 'eq' && ['id', 'displayName', 'externalId'].includes(getObj.attribute)) {
      // mandatory - unique filtering - single unique user to be returned - correspond to getUser() in versions < 4.x.x
      if (getObj.attribute === 'id') path = `/Groups/${getObj.value}?attributes=${attributes.join()}`;
      // GET /Users/bjensen?attributes=
      else
        path = `/Groups?filter=${getObj.attribute} eq "${getObj.value}"${
          attributes.length > 0 ? '&attributes=' + attributes.join() : ''
        }`; // GET /Users?filter=userName eq "bjensen"&attributes=userName,active,name.givenName,name.familyName,name.formatted,title,emails,phoneNumbers,entitlements
        ...
        options: {
          ...
          headers: {
            ...
            Authorization: ctx?.request?.header?.authorization
              ...
              : 'Basic ' +
                Buffer.from(
                  `${config.entity[baseEntity].username}:${scimgateway.getPassword(
      ...
      if (config.entity[baseEntity].proxy && config.entity[baseEntity].proxy.host) {
        ...
        if (config.entity[baseEntity].proxy.username && config.entity[baseEntity].proxy.password) {
          param.options.headers['Proxy-Authorization'] =
            'Basic ' +
            Buffer.from(
              `${config.entity[baseEntity].proxy.username}:${scimgateway.getPassword(
...
const doRequest = async (baseEntity, method, path, body, ctx, opt, retryCount) => {
  try {
    ...
    scimgateway.logger.debug(
      `${pluginName}[${baseEntity}] doRequest ${method} ${options.protocol}//${options.host}${
        options.port ? `:${options.port}` : ''
      }${path} Body = ${JSON.stringify(body)} Response = ${JSON.stringify(result)}`,
    );
    return result;
  } catch (err) {
    // includes failover/retry logic based on config baseUrls array
    scimgateway.logger.error(
      `${pluginName}[${baseEntity}] doRequest ${method} ${path} Body = ${JSON.stringify(body)} Error Response = ${
        err.message
      }`,
    );
    let statusCode;
    ...
    if (!urlObj && (err.code === 'ECONNREFUSED' || err.code === 'ENOTFOUND')) {
      if (retryCount < config.entity[baseEntity].baseUrls.length) {
        ...
        scimgateway.logger.debug(
          ...
          }retry[${retryCount}] using baseUrl = ${_serviceClient[baseEntity].baseUrl}`,
        );
