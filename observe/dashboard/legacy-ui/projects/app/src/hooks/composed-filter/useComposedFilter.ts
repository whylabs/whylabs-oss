...
import { CURRENT_FILTER } from 'types/navTags';
import { useDeepCompareEffect } from '../useDeepCompareEffect';
import { ComposedFilter, ComposedFilterDimension, FiltersList, ReadyToQueryFilter } from './types';
...
export const useComposedFilter = ({
  ...
  const addFilter = (newFilter: FiltersList[number] = {}) => {
    setComposedFilters((prev) => [...prev, newFilter]);
  };
...
function readComposedFiltersInitialState(filterString: string | null): FiltersList {
  ...
  try {
    return JSON.parse(filterString) ?? defaultValue;
  } catch (error) {
