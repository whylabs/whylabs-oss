...
describe('<WhyLabsAutocomplete />', () => {
  ...
  it('should call onChange callback when value changes', () => {
    ...
    userEvent.type(getByRole('combobox'), searchText);
    expect(onChange).toHaveBeenCalledTimes(searchText.length);
  });

  it('should call onChange callback when an option is clicked', () => {
  ...
  it('should filter options when a value is typed', () => {
    const data = ['Adrain', 'Jany', 'Brandon'];
    getRenderer({ data });

    userEvent.type(getByRole('combobox'), 'ra');
  ...
  it("should display 'no results' message when no options are available", () => {
    const data = ['Adrain', 'Jany', 'Brandon'];
    getRenderer({ data, nothingFound: 'No results' });

    userEvent.paste(getByRole('combobox'), 'something else');
