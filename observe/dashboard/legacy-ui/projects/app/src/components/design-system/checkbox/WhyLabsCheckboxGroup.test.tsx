...
import WhyLabsCheckboxGroup, { WhyLabsCheckboxGroupProps, WhyLabsCheckboxGroupOptions } from './WhyLabsCheckboxGroup';
...
describe('<WhyLabsCheckboxGroup />', () => {
  ...
  describe('onChange callback', () => {
    it('should call it when a checkbox is selected', () => {
      const options = getOptionsList();
      ...
      userEvent.click(getByRole('checkbox', { name: options[0].label?.toString() }));
      expect(onChange).toHaveBeenCalledWith([options[0].value]);

      userEvent.click(getByRole('checkbox', { name: options[2].label?.toString() }));
