/* Generates an anomaly if the number of missing values for input features in the target batch differs by more than 50% of the number in a trailing window
...
change monitor displayName to "Missing value ratio percent diff"
...
const presetName = 'Missing values ratio percent monitor with trailing window baseline';
const displayName = 'Missing values ratio percent diff';
const baseConfig = {
  schedule: {
    type: 'fixed',
...
const description: JSX.Element = (
  <>
    ...
    for any input column in the target batch differs by more than <pre>threshold</pre> percent of the ratio in a
    trailing window.
  </>
);
...
export const missingValuesTrailingWindowPreset: JsonPreset = {
  displayName,
  baseConfig,
  programmaticChanges,
  presetName,
