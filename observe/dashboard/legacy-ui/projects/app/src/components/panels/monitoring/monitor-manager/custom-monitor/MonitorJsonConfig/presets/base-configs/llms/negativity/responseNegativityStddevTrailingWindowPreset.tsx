...
const presetName = 'Response negativity detection (stddev) monitor with trailing window baseline';
const displayName = 'Response negativity detection';
const baseConfig = {
  schedule: {
    type: 'fixed',
  ...
  targetMatrix: {
    type: 'column',
    include: ['response.sentiment.sentiment_score'],
    exclude: [],
    segments: [],
  },
  config: {
...
const description: JSX.Element = (
  <>
    ...
    negative words in response from the target batch differs by more than <pre>factor</pre> standard deviations from the
    value in a trailing window.
  </>
);
...
export const responseNegativityStddevTrailingWindowPreset: JsonPreset = {
  displayName,
  baseConfig,
  programmaticChanges,
  presetName,
