...
const presetName = 'Response toxicity detection (stddev) monitor with trailing window baseline';
const displayName = 'Response toxicity detection';
const baseConfig = {
  schedule: {
    type: 'fixed',
  ...
  targetMatrix: {
    type: 'column',
    include: ['response.toxicity.toxicity_score'],
    exclude: [],
    segments: [],
  },
  config: {
...
const description: JSX.Element = (
  <>
    ...
    response toxicity in the target batch differs by more than <pre>factor</pre> standard deviations from the value in a
    trailing window.
  </>
);
...
export const responseToxicityStddevTrailingWindowPreset: JsonPreset = {
  displayName,
  baseConfig,
  programmaticChanges,
  presetName,
