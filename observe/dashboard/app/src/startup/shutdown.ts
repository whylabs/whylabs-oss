...
import { DashbirdContext } from './graphql-server';
 ...
 * @param logger
 */
export const setupGracefulExits = (
  server: Server,
  apolloServer: ApolloServer<DashbirdContext> | null,
  logger: pino.Logger,
): void => {
  const shutdown = async (defaultCode = 0): Promise<void> => {
    ...
    if (apolloServer) {
      ...
      // TODO: check on this thread: https://github.com/apollographql/apollo-server/issues/4097
      // currently this doesnt seem to properly stop Apollo server in some cases
      await apolloServer.stop();
      logger.info('Apollo successfully exited, stopping Express.');
    }
