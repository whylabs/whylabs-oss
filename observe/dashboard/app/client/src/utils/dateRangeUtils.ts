...
export const calculateDailyTrailingWindow = (size: number, endDate = new Date()): [Date, Date] => {
  const endCopy = new Date(endDate);
  const end = setEndOfUTCDay(endCopy);
  const start = setStartOfUTCDay(endCopy.setUTCDate(endCopy.getUTCDate() - size));
  return [start, end];
};
