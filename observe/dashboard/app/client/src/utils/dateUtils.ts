...
import { addDays, formatDistance, isAfter, isBefore, isEqual, isValid } from 'date-fns';
...
export const formatUtcTimestamp = (date: string | number | Date, fmt: string) =>
  format(utcToZonedTime(date, 'UTC'), fmt, { timeZone: 'UTC' });

function universalFormat(timestamp: number, dateFormat: string) {
  // eslint-disable-next-line no-restricted-globals
  if (!timestamp || isNaN(timestamp)) {
    LogRocket.error("universalFormat: can't format invalid timestamp");
    return '';
  }
