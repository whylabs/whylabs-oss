...
import { ExternalLinkKeys } from '~/constants/externalLinks';
import { SegmentedRowComponent } from '~/routes/:orgId/:resourceId/llm-trace/policy/components/SegmentedRowComponent';
...
const RULESET_DOCUMENTATION_MAP: Record<string, ExternalLinkKeys> = {
  'bad-actors': 'badActorsDocumentationUrl',
  misuse: 'misuseDocumentationUrl',
  cost: 'costDocumentationUrl',
  'customer-experience': 'customerExperienceDocumentationUrl',
  truthfulness: 'truthfulnessDocumentationUrl',
};
...
export const PolicyExpandableCard = ({
  ...
  const disabledTooltip = disabled
    ? `The ${title} ruleset is configured via YAML in Advanced Settings. To reenable it via the UI, you must remove its YAML config.`
    : '';
  ...
  const renderPolicyLearnMoreText = () => {
    const documentationUrl = RULESET_DOCUMENTATION_MAP[id];
    const rulesetLink = documentationUrl ? <ExternalLink to={documentationUrl}>ruleset</ExternalLink> : 'ruleset';
    const guardrailMetricLink = <ExternalLink to="secureGuardrailMetrics">guardrail metrics</ExternalLink>;

    return (
      <WhyLabsText className={classes.descriptionText}>
        Learn more about this {rulesetLink} and the {guardrailMetricLink} that contribute to it.
      </WhyLabsText>
    );
  };
    ...
    <WhyLabsAccordion.Item value={id} className={classes.policyCardRoot}>
      <div className={classes.switchWrapper}>
        <WhyLabsSwitch
          ...
          checked={isEnabledPolicy}
          label="Toggle policy"
          hideLabel
          disabled={disabled}
          disabledTooltip={disabledTooltip}
          backgroundColor={disabled ? Colors.disabledChartPrimary : Colors.chartPrimary}
        />
      </div>
