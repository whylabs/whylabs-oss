...
import { UsePieChartBuilderViewModelProps, usePieChartBuilderViewModel } from './usePieChartBuilderViewModel';
import { useGraphBuilderStyles } from './utils';

type PieChartBuilderProps = UsePieChartBuilderViewModelProps & {
  index: number;
  onClone: () => void;
  onDelete: () => void;
};

export const PieChartBuilder = ({ index, onClone, onDelete, ...rest }: PieChartBuilderProps) => {
  const { column, displayName, isResourceEmpty, isItReady, resource, segment } = usePieChartBuilderViewModel(rest);
    ...
    <div className={classes.root}>
      ...
      <div className={classes.metricSelectWrapper}>
        <WhyLabsSelect
          ...
          data={['Frequent Items']}
          disabled
          disabledTooltip="Pie Chart only support frequent items."
          itemComponent={SelectCustomItems.GenericFlexColumnItem}
          label="Metric:"
