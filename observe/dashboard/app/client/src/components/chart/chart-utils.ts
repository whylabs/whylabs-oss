...
const getColumnPlotOptions = (
  grouping?: boolean,
  stacking: OptionsStackingValue | null = 'normal',
): PlotColumnOptions => {
  const commonOptions = {
    shadow: false,
    borderWidth: 0,
...
export const getChartCommonOptions = ({
  ...
  plotOptions: {
    series: {
      ...
      // dataLabels: { put it back to display labels on each data-point of series
      //   enabled: true,
      //   format: '{point.y:.4f}',
    ...
    column: getColumnPlotOptions(spec?.plotOptions?.columnGrouping, spec?.plotOptions?.columnStackingMode),
    line: {
      marker: {
        enabled: true,
        radius: 3,
...
/** Extracted form HCPlaceholder default patterns options.
 * To customize the color we ned to use the HCPlaceholder.merge(GraphStripedPattern, { color: 'your-color' })
 */
export const GraphStripedPattern = {
