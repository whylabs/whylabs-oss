...
import { ComposedFilterOption } from '~/hooks/composed-filter/types';
import { useComposedFilter } from '~/hooks/composed-filter/useComposedFilter';
import { handlePlural } from '~/utils/stringUtils';
import { isString } from '~/utils/typeGuards';
import { EXCLUDES_CONDITION, INCLUDES_CONDITION } from '~server/util/composed-filters-utils';
        ...
        <div className={classes.buttonBadgesContainer}>
          {readyToQueryFilters.map((f, index) => {
            ...
            const [label, selectedOptions] = (() => {
              ...
              const fallbackCondition = selectedDimension?.type === 'string' ? 'contains' : '=';
              return [`${dimensionLabel} ${f.condition ?? fallbackCondition} ${f.value}`, [f.value]];
            })();
            ...
            const mappedItems = selectedOptions.map((item) => (isString(item) ? { value: item } : item));
            const customComponent = renderFilterCustomBadges?.(mappedItems, f.dimension, defaultFilterBadge, index);
            return <Fragment key={label}>{customComponent}</Fragment>;
          })}
        </div>
      );
    }
