...
it('should add 1 millisecond to date object', () => {
  const input = new Date('2024-11-11T23:59:59.999Z');
  const expected = addOneMillisecondToDate(input);
  expect(input).not.toEqual(expected); // not the same instance
  expect(expected.toISOString()).toEqual('2024-11-12T00:00:00.000Z');
});
