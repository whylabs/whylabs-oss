...
describe('<WhyLabsTextInput />', () => {
  ...
  it.each(['text', 'email', 'password', 'tel', 'url', 'number', 'date'])(`should have type %p`, (type) => {
    getRenderer({ type });
  ...
  describe('onChange callback', () => {
    ...
    it('should call it with typed values', async () => {
      const onChange = jest.fn();
      getRenderer({ onChange });
...
// Helpers
function getRenderer({ label = DEFAULT_LABEL, ...rest }: Partial<WhyLabsTextInputProps> = {}) {
  return render(
    <MantineProvider>
      <WhyLabsTextInput label={label} {...rest} />
    </MantineProvider>,
  );
}
