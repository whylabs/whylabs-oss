...
import WhyLabsRadioGroup, { WhyLabsRadioGroupOptions, WhyLabsRadioGroupProps } from './WhyLabsRadioGroup';
...
describe('<WhyLabsRadioGroup />', () => {
  ...
  describe('onChange callback', () => {
    it('should call it when a radio is selected', async () => {
      const options = getOptionsList();
      ...
      await userEvent.click(getByRole('radio', { name: options[0].label?.toString() }));
      expect(onChange).toHaveBeenCalledWith(options[0].value);

      await userEvent.click(getByRole('radio', { name: options[2].label?.toString() }));
