...
import WhyLabsSearchInput, { WhyLabsSearchInputProps } from './WhyLabsSearchInput';

const { getByLabelText, getByPlaceholderText, getByRole, getByTestId, getByText, queryByText } = screen;

const TEST_ID = 'WhyLabsSearchInput';
const DEFAULT_LABEL = 'Input';
...
// Helpers
function getRenderer({ label = DEFAULT_LABEL, ...rest }: Partial<WhyLabsSearchInputProps> = {}) {
  return render(
    <MantineProvider>
      <WhyLabsSearchInput label={label} {...rest} />
    </MantineProvider>,
  );
}
