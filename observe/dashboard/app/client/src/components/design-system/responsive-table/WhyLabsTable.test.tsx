...
const defaultLinkColumn = (
  <WhyLabsTable.Column
    key="test-link"
    columnKey="test-link"
    header={<HeaderCell columnKey="test-link">{`${TEST_COLUMNS[1]} header`}</HeaderCell>}
...
describe('<WhyLabsTable />', () => {
  ...
  it('should render a table with 3 columns, 3 rows and 6 cells', () => {
    const columns = [
      <WhyLabsTable.Column
        key="id"
        columnKey="id"
        header={<HeaderCell columnKey="id">ID</HeaderCell>}
        cell={(rowNumber) => <TextCell>{rowNumber}</TextCell>}
      />,
      <WhyLabsTable.Column
        key="name"
        columnKey="name"
        header={<HeaderCell columnKey="name">Name</HeaderCell>}
        cell={(rowNumber) => <TextCell>Someone {rowNumber}</TextCell>}
      />,
      <WhyLabsTable.Column
        key="age"
        columnKey="age"
        header={<HeaderCell columnKey="age">Age</HeaderCell>}
  ...
  describe("children's cell", () => {
    it.each([1, 3])("should pass isSelected prop to children's cell %#", (selectedRow) => {
      ...
      const columns = [
        <WhyLabsTable.Column key="id" columnKey="id" header={<>ID</>} cell={renderCell1} />,
        <WhyLabsTable.Column key="name" columnKey="name" header={<>Name</>} cell={renderCell2} />,
      ];
      const rowsCount = 4;
      ...
      arrayOfLength(rowsCount).forEach((rowIndex) => {
        expect(renderCell1).toHaveBeenNthCalledWith(rowIndex + 1, rowIndex, {
          isSelected: selectedRow === rowIndex,
          columnKey: 'id',
        });
        expect(renderCell2).toHaveBeenNthCalledWith(rowIndex + 1, rowIndex, {
          isSelected: selectedRow === rowIndex,
          columnKey: 'name',
        });
      });
