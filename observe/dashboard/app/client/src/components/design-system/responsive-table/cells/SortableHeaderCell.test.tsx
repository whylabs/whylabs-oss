...
describe('<SortableHeaderCell />', () => {
  ...
  describe('onSortDirectionChange callback', () => {
    describe('sortType="text"', () => {
      it('should call it with Asc when clicked and no sort applied', async () => {
        const onSortDirectionChange = jest.fn();
        getRenderer({ sortDirection: undefined, sortType: 'text', onSortDirectionChange });

        await userEvent.click(getByRole('button', { name: DEFAULT_BUTTON_LABEL }));
        expect(onSortDirectionChange).toHaveBeenCalledWith(SortDirection.Asc);
      });

      it('should call it with Desc when clicked and sort is Asc', async () => {
        const onSortDirectionChange = jest.fn();
        getRenderer({ sortDirection: SortDirection.Asc, sortType: 'text', onSortDirectionChange });

        await userEvent.click(getByRole('button', { name: ALPHABETICALLY_BUTTON_LABEL }));
        expect(onSortDirectionChange).toHaveBeenCalledWith(SortDirection.Desc);
      });

      it('should call it with undefined when clicked and sort is Desc', async () => {
        const onSortDirectionChange = jest.fn();
  ...
  describe('sortType="number"', () => {
    it('should call it with Desc when clicked and no sort applied', async () => {
      const onSortDirectionChange = jest.fn();
    ...
    it('should call it with Asc when clicked and sort is Desc', async () => {
      const onSortDirectionChange = jest.fn();
      getRenderer({ sortDirection: SortDirection.Desc, sortType: 'number', onSortDirectionChange });

      await userEvent.click(getByRole('button', { name: DESCENDING_BUTTON_LABEL }));
      expect(onSortDirectionChange).toHaveBeenCalledWith(SortDirection.Asc);
    });

    it('should call it with undefined when clicked and sort is Asc', async () => {
      const onSortDirectionChange = jest.fn();
      getRenderer({ sortDirection: SortDirection.Asc, sortType: 'number', onSortDirectionChange });

      await userEvent.click(getByRole('button', { name: ASCENDING_BUTTON_LABEL }));
      expect(onSortDirectionChange).toHaveBeenCalledWith(undefined);
    });
  });
});
