...
export const mapTimePeriodToPresetGranularity = new Map<TimePeriod, PresetGranularity>([
...
export const useDynamicTrailingRangePresets = (batchFrequency: TimePeriod) => {
...
export const convertRelativePresetToRange = (preset: string): [Date, Date] | null => {
  const [granularity, size] = preset.split(RELATIVE_PRESET_TOKEN);
  const timePeriod = mapPresetGranularityToTimePeriod.get(granularity);
  const trailingWindowCalculator = timePeriod && mapTrailingWindowHandler.get(timePeriod);
  if (!trailingWindowCalculator || !isValidNumber(Number(size))) return null;
  return trailingWindowCalculator(Number(size));
};
