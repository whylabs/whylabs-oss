...
export const SuperDateRangeSelector = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      setRangeParams({ presetWindow: CUSTOM_RANGE });
    }
  ...
  const onPresetSelect = (selected: string) => {
    ...
    const isRelativePreset = selected.includes(RELATIVE_PRESET_TOKEN);
    ...
    if (newStartDate && newEndDate) {
      ...
      setRangeParams({
        start: isRelativePreset ? null : dateConstructorToReadableISOString(newStartDate?.getTime() ?? null),
        end: isRelativePreset ? null : dateConstructorToReadableISOString(newEndDate?.getTime() ?? null),
