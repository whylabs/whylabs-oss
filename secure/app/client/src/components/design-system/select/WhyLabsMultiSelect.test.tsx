...
describe('<WhyLabsMultiSelect />', () => {
  ...
  it('should allow user to type in the searchbox', async () => {
    ...
    const selectElement = getByLabelText(DEFAULT_LABEL);
    await userEvent.type(selectElement, 'so patate', USER_EVENT_CONFIG);
  ...
  it('should have options', async () => {
    ...
    getRenderer({ data });
    await userEvent.click(getByLabelText(DEFAULT_LABEL), USER_EVENT_CONFIG);
  ...
  it('should not have a clear button when clearable=false', () => {
    const data = defaultList();
  ...
  it('should display default nothing found message', async () => {
    const data = defaultList();

    getRenderer({ data });
    await userEvent.type(getByLabelText(DEFAULT_LABEL), `something wrong`, USER_EVENT_CONFIG);
    expect(getByText('Nothing found')).toBeInTheDocument();
  });

  it.each(['nothing', 'nothing to display'])('should display custom nothing found message %p', async (expected) => {
    const data = defaultList();
  ...
  describe('while loading is true', () => {
    ...
    it("should have 'Loading...' as nothing found message", async () => {
      getRenderer({ loading: true });
  ...
  describe('onChange callback', () => {
    it('should call it when the user selects an option', async () => {
      ...
      await userEvent.click(getByRole('searchbox', { name: DEFAULT_LABEL }), USER_EVENT_CONFIG);
      await userEvent.click(getByRole('option', { name: data[0].label }), USER_EVENT_CONFIG);
      expect(onChange).toHaveBeenCalledTimes(1);

      await userEvent.click(getByRole('option', { name: data[2].label }), USER_EVENT_CONFIG);
    ...
    it('should call it when the user clears the value', async () => {
      const onChange = jest.fn();
      const data = defaultList();
      ...
      // TODO: this is bad, Mantine classes can change and break the test
      // Must change it to query by button role instead
      const closeBtn = container.querySelector('div.mantine-Input-rightSection button.mantine-CloseButton-root');
      if (closeBtn) {
        await userEvent.click(closeBtn, USER_EVENT_CONFIG);
      }
    ...
    it('should call it when user type in the searchbox and select the item using keyboard', async () => {
      const onChange = jest.fn();
      const data = defaultList();
      getRenderer({ data, onChange });

      await userEvent.type(getByLabelText(DEFAULT_LABEL), `${data[1].label}{arrowdown}{enter}`, USER_EVENT_CONFIG);
