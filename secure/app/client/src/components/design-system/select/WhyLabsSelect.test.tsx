...
describe('<WhyLabsSelect />', () => {
  ...
  it('should allow user to type in the searchbox', async () => {
    ...
    await userEvent.type(selectElement, 'so patate');
  ...
  it('should display default nothing found message', async () => {
    ...
    getRenderer({ data });
    await userEvent.type(getByRole('searchbox'), `something wrong`);
  ...
  it.each(['A tooltip', 'Another tooltip'])('should display label tooltip', async (expected) => {
    getRenderer({ label: 'Something', labelTooltip: expected });

    await userEvent.hover(getByRole('button', { name: '?' }));
  ...
  describe('onChange callback', () => {
    it('should call it when the user selects an option', async () => {
      ...
      await userEvent.click(getByLabelText(DEFAULT_LABEL));
      await userEvent.click(getByRole('option', { name: data[0].label }));
      expect(onChange).toHaveBeenCalledTimes(1);

      await userEvent.click(getByLabelText(DEFAULT_LABEL));
      await userEvent.click(getByRole('option', { name: data[2].label }));
    ...
    it('should call it when user type in the searchbox and select the item using keyboard', async () => {
      ...
      await userEvent.type(getByRole('searchbox'), `${data[1].label}{arrowdown}{enter}`);
...
function getRenderer({ data = [], value, label = DEFAULT_LABEL, ...rest }: Partial<Props> = {}) {
  return render(
    <MantineProvider>
      <WhyLabsSelect data={data} label={label} value={value ?? null} {...rest} />
    </MantineProvider>,
  );
}
