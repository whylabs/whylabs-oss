...
import { WhyLabsButton, WhyLabsMultiSelect, WhyLabsMultiSelectProps } from '~/components/design-system';
...
const KeyValueMultiSelect = ({
  ...
  const showAmpersand: WhyLabsMultiSelectProps['concatenationToken'] = (() => {
    if (!selectInFocus || (selectedPeers?.length && !data.length)) {
      return { token: '&', position: 'between' };
    }
    return { token: '&', position: 'afterEach' };
  })();
