...
def test_webhook_failures_dont_ruin_run():
    wf = Workflow(
        ...
        validators=[validator_lib.constraint(target_metric="prompt.stats.char_count", upper_threshold=5)],
        callbacks=[callback_lib.webhook.basic_validation_failure(url="https://foo.bar")],  # will fail, url doesn't exist
    )
