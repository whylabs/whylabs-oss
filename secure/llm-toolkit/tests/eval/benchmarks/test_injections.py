...
def test_injections_scorer():
    results = injections_benchmark.run(DummyScorer())
    assert "jb_bench" in results.get_group_names()
    assert "ivanleomk" in results.get_group_names()
