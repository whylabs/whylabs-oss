...
class MedicalScorer(Scorer):
    def __init__(self, tag: str, encoder_name: SentenceTransformerEncoder, version: Optional[int] = None):
        super().__init__(dataset="medical", tag=tag, version=version, encoder_name=encoder_name)
    ...
    def evaluate(self) -> ResultGroup:
        ...
        medical_benchmark = TopicsBenchmark(auto_threshold=True, topic="medical", n=5000)
        results = medical_benchmark.run(self)
        return results


class MedicalScorerChroma(ChromaScorer):
    # Use chromaDB
    def __init__(self, tag: str, encoder_name: SentenceTransformerEncoder, version: Optional[int] = None, neighbors_num: int = 1):
        super().__init__("medical", tag=tag, encoder_name=encoder_name, version=version)
