...
def _get_chromadb_asset_id(label: Optional[Labels]) -> AssetId:
    base_id = "chromadb_twoclass"
    if label is None:
        return cast(AssetId, base_id)
    elif label.name == "code":
        return cast(AssetId, f"{base_id}_code")
    elif label.name == "medical":
        return cast(AssetId, f"{base_id}_medical")
    elif label.name == "financial":
        return cast(AssetId, f"{base_id}_financial")
    elif label.name == "hate":
        return cast(AssetId, f"{base_id}_hate")
    elif label.name == "toxic":
        return cast(AssetId, f"{base_id}_toxic")
    elif label.name == "harmful":
        return cast(AssetId, f"{base_id}_harmful")
    elif label.name == "innocuous":
        return cast(AssetId, f"{base_id}_innocuous")
    else:
        raise ValueError(f"chromadb asset id: Invalid label: {label}")


class ChromaTwoClassVectorDB:
    def __init__(
        self,
        ...
        self._neighbors_num = neighbor_num or 10
    ...
    def _calculate_score(self, query_results: ValidatedQueryResult) -> Sequence[float]:
        ...
        for distance, _metadata in zip(query_results.distances, query_results.metadatas):
            if settings.USE_SIGNAL_COEFFICIENT:
                ...
                similarities = np.maximum(1 - np.array(distance), 0)
                is_positive = [int(m["is_positive"]) for m in _metadata]
                adjusted_labels = [2 * label - 1 for label in is_positive]
                adjusted_similarities = [similarity * label for similarity, label in zip(similarities, adjusted_labels)]
                final_similarities = [(similarity + 1) / 2 for similarity in adjusted_similarities]
                avg_score = np.mean(final_similarities).tolist()
