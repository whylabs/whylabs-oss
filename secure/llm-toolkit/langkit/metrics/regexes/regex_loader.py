...
def _is_pattern(obj: Any) -> bool:
    return (
        ...
        and all(isinstance(expr, str) for expr in obj["expressions"])  # pyright: ignore[reportUnknownVariableType]
    )


def _is_pattern_group(obj: Any) -> bool:
    return isinstance(obj, list) and all(_is_pattern(item) for item in obj)  # pyright: ignore[reportUnknownVariableType]
