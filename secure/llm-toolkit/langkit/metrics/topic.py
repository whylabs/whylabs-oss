...
def __get_scores_per_label(
    ...
    result: List[ClassificationResults] = classifier(  # pyright: ignore[reportAssignmentType]
        sanitized_text, topics, hypothesis_template=hypothesis_template, multi_label=multi_label, device=classifier.device
    )
    return result
