    ...
    def _metric(options: WorkflowOptions) -> Metric:
        ...
        def udf(text: pd.DataFrame, context: Context) -> MultiMetricResult:
            ...
            similarity_injection_results = [
                0.8 * score if label != Labels.injection else score for label, score in zip(labels, chroma_result.metrics)
            ]
            if filter_innocuous_arg:
...
prompt_injections_metric_chroma = partial(injections_metric_chroma, "prompt")
